...
##项目概述
本项目是一个基于 Spring Boot 的教育管理系统，借助 Spring Boot 框架、MyBatis 持久层框架和 MySQL 数据库，构建出一套完整的教育管理解决方案。系统涵盖了学生、教师等角色的相关功能，像学生查看已修课程、教师管理课程与成绩等。
##项目结构
plaintext
...
##修改密码：访问 /teacher/passwordRest 进入密码修改页面。

##日志配置
项目采用 Logback 作为日志框架，日志配置文件为 src/main/resources/logback.xml。日志输出格式如下：
